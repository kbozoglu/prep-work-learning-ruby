...
def caesar_cipher(offset, string)
  ...
  while i < string.length
    if (string[i] != " ") && (string[i].ord + offset <= 122)
      new_str += (string[i].ord + offset).chr
    elsif (string[i] != " ") && (string[i].ord + offset > 122)
      new_str += (string[i].ord + offset - 26).chr
    else
      new_str += string[i]
    end
    i +=1
